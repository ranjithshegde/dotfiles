...
-- function settings.debugger()

    -- dap.adapters.cpp = {
    --     type = 'executable',
    --     name = "cppdbg",
    --     command = vim.api.nvim_get_runtime_file(
    --         "gadgets/linux/vscode-cpptools/debugAdapters/OpenDebugAD7", false)[1], --  // ensure that vimspector is located inside the nvim runtime folde
    --     args = {},
...
-- settings.gdb_c = function(args, mi_mode, mi_debugger_path)

    -- local last_gdb_config
    -- dap.defaults.fallback.external_terminal = {command = '/usr/bin/st', args = {'-e'}}
    -- local env = function()
    --     local variables = {}
    --     for k, v in pairs(vim.fn.environ()) do
    --         table.insert(variables, string.format("%s=%s", k, v))
...
-- end
    ...
    -- Cmd [[
    -- command! -complete=file -nargs=* DebugC lua require('settings').gdb_c({<f-args>}, "gdb")
-- ]]
